...
export default function BookingModal({ isOpen, onClose, expert }: Props) {
  ...
  const handleCashPayment = async () => {
    ...
    try {
      ...
      const appointmentPayload = {
        ...
        status: "confirmed",
        type: serviceType,
        notes: `Cash payment | Package: ${pkg.name} | Sessions: ${pkg.sessions} | Mode: ${patient.mode} | Referral: ${patient.referral || "N/A"}`,
      };

      const appointRes = await fetch(`${API_BASE}/api/appointments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(appointmentPayload),
      });

      if (!appointRes.ok) throw new Error("Failed to save appointment");
      ...
      const cashRes = await fetch(`${API_BASE}/api/payment/cash-payment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(cashPayload),
      });

      if (!cashRes.ok) throw new Error("Failed to record cash payment");

      alert("✅ Payment successful (Cash). Appointment booked!");
      onClose();
      resetForm();
    } catch (err: any) {
      console.error(err);
      alert(err.message || "Cash payment failed. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleOnlinePayment = async () => {
    ...
    try {
      ...
      const options = {
        key: "rzp_test_RBxndd6SfS8vVC", // 🔁 Replace with your live/test key
        amount: orderData.amount,
        currency: "INR",
        name: "Oppam Therapy",
        description: "Appointment Booking",
        ...
        handler: async function (response: any) {
          try {
            ...
            const appointmentPayload = {
              ...
              status: "confirmed",
              type: serviceType,
              notes: `Payment ID: ${response.razorpay_payment_id} | Package: ${pkg.name} | Sessions: ${pkg.sessions}`,
            };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 rounded-xl p-4 space-y-3">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium">₹{booking.package?.price.toLocaleString()}</span>
        </div>
        <div className="flex items-center justify-between gap-2">
            ...
            {[1, 2, 3, 4, 5].map((s) => (
              <div key={s} className="flex-1 text-center">
                ...
                <div className="text-xs mt-1 text-gray-500">
                  {s === 1 ? "Service" : s === 2 ? "Package" : s === 3 ? "DateTime" : s === 4 ? "Details" : "Payment"}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-between mt-8">
            ...
            {!isLastStep ? (
              <button
                onClick={handleNext}
                disabled={
                  (step === 1 && !booking.serviceType) ||
                  (step === 2 && !booking.package) ||
                  (step === 3 && (!booking.date || !booking.timeSlot)) ||
                  (step === 4 && (!booking.patient.name || !booking.patient.email || !booking.patient.whatsapp || !booking.patient.mode))
                }
                className="ml-auto px-5 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:opacity-50"
              >
                Continue
              </button>
            ) : (
